...
extern float BLE_filtered_x;
extern float BLE_filtered_y;
extern float BLE_filtered_z;
extern int BLE_filtered_new;
...
void MX_BlueNRG_MS_Init(void)
{
  ...
  if (ret == BLE_STATUS_SUCCESS){
      PRINTF("Custom BLE Service added.\n");}
  else{
      PRINTF("Error adding Custom BLE Service: 0x%02x\r\n", ret);
  	  while(1);
  }
...
static void User_Process(void)
{
  ...
  if (user_button_pressed)
  {
    ...
    if (connected)
    {
      ...
      if(BLE_filtered_new){
      PRINTF("x_out: %2f ,y_out: %2f,z_out: %2f\n",BLE_filtered_x, BLE_filtered_y, BLE_filtered_z);

      Update_FilteredData(BLE_filtered_x, BLE_filtered_y, BLE_filtered_z);
      BLE_filtered_new = 0;
