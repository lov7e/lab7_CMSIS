...
#define COPY_CUSTOM_SERVICE_UUID(uuid_struct)  COPY_UUID_128(uuid_struct, 0xAA,0xBB,0xCC,0xDD,0xEE,0xFF,0x11,0x22,0x33,0x44,0x55,0x66,0x00,0x00,0x00,0x01)
#define COPY_FILTERED_DATA_CHAR_UUID(uuid_struct) COPY_UUID_128(uuid_struct, 0xAA,0xBB,0xCC,0xDD,0xEE,0xFF,0x11,0x22,0x33,0x44,0x55,0x66,0x00,0x00,0x00,0x02)


uint16_t CustomServiceHandle, FilteredCharHandle;
...
tBleStatus Add_Custom_Service(void)
{
  ...
  BLUENRG_memcpy(service_uuid.Service_UUID_128, uuid, 16);
  ret = aci_gatt_add_serv(UUID_TYPE_128, service_uuid.Service_UUID_128, PRIMARY_SERVICE,
                          1 + 3, &CustomServiceHandle);
  if (ret != BLE_STATUS_SUCCESS) return BLE_STATUS_ERROR;

  // === 2. Characteristic ===
  COPY_FILTERED_DATA_CHAR_UUID(uuid);
  BLUENRG_memcpy(char_uuid.Char_UUID_128, uuid, 16);
  ret = aci_gatt_add_char(CustomServiceHandle, UUID_TYPE_128, char_uuid.Char_UUID_128,
                          12, // 3 floats x 4 bytes
                          CHAR_PROP_NOTIFY,
                          ATTR_PERMISSION_NONE,
                          GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
                          16, 0, &FilteredCharHandle);
  if (ret != BLE_STATUS_SUCCESS) return BLE_STATUS_ERROR;

  return BLE_STATUS_SUCCESS;
}

tBleStatus Update_FilteredData(float x, float y, float z)
{
    uint8_t buff[12];
    memcpy(buff,     (uint8_t*)&x, 4);
    memcpy(buff + 4, (uint8_t*)&y, 4);
    memcpy(buff + 8, (uint8_t*)&z, 4);

    return aci_gatt_update_char_value(CustomServiceHandle, FilteredCharHandle,
                                      0, sizeof(buff), buff);
}
